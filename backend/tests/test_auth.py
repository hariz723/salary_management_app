...
def test_login_success(client, seeded_db):
    payload = {"email": "hr.manager@acme.com", "password": "Password123"}
...
def test_login_invalid_password(client, seeded_db):
    payload = {"email": "hr.manager@acme.com", "password": "WrongPassword"}
    response = client.post("/api/v1/auth/login", json=payload)
...
def test_get_current_user_me(client, seeded_db):
    ...
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": "hr.manager@acme.com", "password": "Password123"}
    )
