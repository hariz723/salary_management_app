...
class SalaryRepository:
    ...
    def get_current_by_employee_id(self, employee_id: str) -> SalaryRecord | None:
        return (
            ...
            .filter(SalaryRecord.employee_id == employee_id, SalaryRecord.is_current == True)
            .first()
        )
    ...
    def get_history_by_employee_id(self, employee_id: str) -> list[SalaryRecord]:
        return (
            ...
            .order_by(desc(SalaryRecord.effective_date), desc(SalaryRecord.created_at))
            .all()
        )
    ...
    def get_exchange_rate(self, currency_code: str) -> ExchangeRate | None:
        return (
            self.db.query(ExchangeRate).filter(ExchangeRate.currency_code == currency_code).first()
        )
