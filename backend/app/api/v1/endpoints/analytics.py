...
@router.get(
    ...
    summary="Get high-level compensation KPIs across ACME",
)
...
@router.get(
    "/distribution",
    response_model=list[DistributionBucket],
    summary="Get compensation distribution buckets",
)
...
@router.get(
    ...
    summary="Get detailed metrics and percentiles per department",
)
...
@router.get(
    "/countries",
    response_model=list[CountryStats],
    summary="Get payroll expenditure per country and currency",
)
...
@router.get(
    "/job-levels",
    response_model=list[JobLevelStats],
    summary="Get compensation breakdown by job level",
)
...
@router.get(
    "/gender-pay-gap",
    response_model=GenderPayGapAnalysis,
    summary="Get comprehensive gender pay parity & diversity metrics",
)
...
@router.get(
    "/band-compliance",
    response_model=BandComplianceSummary,
    summary="Get compensation band outliers and correction budget impact",
)
...
@router.get(
    "/hr-questions",
    response_model=list[HRQuestionCard],
    summary="Get instant answers to strategic HR executive questions",
)
